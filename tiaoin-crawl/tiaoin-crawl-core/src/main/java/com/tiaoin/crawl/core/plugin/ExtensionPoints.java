...
public class ExtensionPoints {
    ...
    public static boolean contains(String name) {
        return task_poll.equals(name) || begin.equals(name) || fetch.equals(name)
               || dig.equals(name) || dup_removal.equals(name) || task_sort.equals(name)
               || task_push.equals(name) || target.equals(name) || parse.equals(name)
               || pojo.equals(name) || end.equals(name);
    }

    public static String string() {
        return "[" + task_poll + ", " + begin + ", " + fetch + ", " + dig + ", " + dup_removal
               + ", " + task_sort + ", " + task_push + ", " + target + ", " + parse + ", " + pojo
               + ", " + end + "]";
    }

    public static Collection<String> toArray() {
        return Arrays.asList(task_poll, begin, fetch, dig, dup_removal, task_sort, task_push,
            target, parse, pojo, end);
    }
}
