...
public class Spider implements Runnable {
    ...
    public void run() {
        try {
            ...
            if (result == null || result.getPage() == null || result.getPage().getContent() == null
                || result.getPage().getContent().trim().length() == 0) {
                listener.onInfo(Thread.currentThread(), task,
                    " spider stop cause the fetch result->+" + result + " of task[" + task
                            + "] is null");
                return;
            }
            ...
            if (page == null) {
                listener.onInfo(Thread.currentThread(), task,
                    " spider stop cause the fetch result.page is null");
                return;
            }
