...
import com.tiaoin.crawl.core.xml.Field;
import com.tiaoin.crawl.core.xml.Parser;
import com.tiaoin.crawl.core.xml.Parsers;
...
public class ModelParser extends DefaultHandler {
    ...
    private void init(Task task, Target target, SpiderListener listener) {
        ...
        fel.getContext().set("$Util", StringUtil.class);
...
//                                                      + ",url->" + page.getUrl());
        ...
        boolean isXml = contentType.contains("text/xml")
    ...
    private Map<String, Object> parseXmlMap(Object item, XPath xpathParser, final List<Field> fields) {
        ...
        for (Field field : fields) {
            ...
            List<Parser> parserList = parsers.getParser();
            ...
            for (int i = 0; i < parserList.size(); i++) {
                ...
                try {
                    ...
                    listener.onError(Thread.currentThread(), task,
                        "key->" + key + " parse failed cause->" + e.toString(), e);
                }
            }
        ...
        if (list == null || list.isEmpty()) {
            ...
            for (Object val : list) {
                ...
                try {
                    ...
                    if (!isValBlank)
                        listener.onError(Thread.currentThread(), task, "exp->" + exp
                                                                       + " eval failed", e);
                } finally {
        ...
        for (Object obj : list) {
            try {
                ...
                if (input == null || input.trim().length() == 0)
                    continue;
                List<String> vals = StringUtil.findByRegex(input, regex);
                //如果REGEX找不到
                if (vals == null) {
                ...
                listener.onError(Thread.currentThread(), task, "regex->" + regex + " of " + obj
                                                               + " parse failed", e);
                if ("1".equals(skipRgxFail) || "true".equals(skipRgxFail))
                    continue;
                newVals.add("");
